...
class models():
	...
	#	cost = y_label * K.log(y_pred) - costMatrix 
		return K.sum(y_cost, axis=1) 


	def trainANN(self, inputSize, dropout, hidden, epoch, learning=0.001, output_size=4):
		model_name = '_Drop'+str(dropout)+'_Hidden'+str(hidden)+'_Epoch'+str(epoch)+'_Learning'+str(learning)
	...
	def trainAutoSK(self, traintime, algorithm):
		modelName = '_time'+str(traintim)+'_algorithm'+str(algorithm)
		model =  autosklearn.regression.AutoSklearnRegressor(time_left_for_this_task=86400)
		print("training model")
		model.fit(self.features, self.y_cost)
