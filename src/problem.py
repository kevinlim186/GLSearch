...
class Problem():
    ...
    def simplexAlgorithm(self, population):
        ...
        minimize(self.problemInstance,tol=1e-10, x0=population, method='nelder-mead', options=opt)
                    ...
                    #ELA features will only be computed if there are models in the model list
                        if model['size']==size:
                            ...
                            if len(elaFeatSize) < stepSize:
                                ...
                                #add additional step in the ela
                                for i in range(2, stepSize+1):   
                                    ela1 = np.array([elaFeatSize[x_labels].iloc[-i,].values]).astype('float32')
                                    ela = np.concatenate((ela1,ela),axis=0)
