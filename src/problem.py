...
class Problem():
    ...
    def calculateELA(self, size=None, sanitize=False):
        if size is None:
            sample = sample.iloc[:,0:self.dimension]
            ...
            sample = self.currentResults.iloc[:,0:self.dimension].values[sampleSize-1:-1]
            obj_values = self.currentResults['y'].values[sampleSize-1:-1]
            featureObj = create_feature_object(sample,obj_values, lower=-5, upper=5)
        ...
        while self.totalBudget > self.spentBudget and not (targetReached):
            ...
            if (checkpoints[currentLength] < self.spentBudget and currentLength < maxIndex):
                ...
                if (index > 0):
                    ...
                    #Check if BFGS 0.1
                    if (index==1):
                        name = self.getProblemName(self.function, self.instance, self.spentBudget,'bfgs0.1'+str(size)+'_'+ASPName,testRun)
                        #self.saveElaFeat(name)
                        self.bfgsAlgorithm(x0, 0.1)
                     ...
                     #Check if BFGS 0.3
                    if (index==2):
                        name = self.getProblemName(self.function, self.instance, self.spentBudget,'bfgs0.3'+str(size)+'_'+ASPName,testRun)
                        #self.saveElaFeat(name)
                        self.bfgsAlgorithm(x0, 0.3)
