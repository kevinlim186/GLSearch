...
class Problem():
    ...
    def calculateELA(self, size=None, sanitize=False):
        if size is None:
            ...
            mask = (self.currentResults.iloc[:,0:self.dimension]>-5) & (self.currentResults.iloc[:,0:self.dimension]<5)
            sampleFiltered = self.currentResults[mask].dropna()
            sample = sampleFiltered.iloc[:,0:self.dimension].values[size-1:-1]
            obj_values = sampleFiltered['y'].values[size-1:-1]
            featureObj = create_feature_object(sample,obj_values, lower=-5, upper=5)
